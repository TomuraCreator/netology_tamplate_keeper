    ...
    if(hrefSplit[hrefSplit.length - 1] === "edit#end_form") {
        ...
        window.addEventListener("click", event => {
            ...
            if(target.classList.contains("_-packages-ui-kit-components-v2-Button--button--27Ont")
                ...
                let wh: boolean | Node = false;
                while(!wh) {
                    wh = new Context().getDocument.querySelector(".components-trainer-TaskPage-components-Header-components-User--userName--1K9q-")
                }
                ...
                port.onMessage.addListener( (msg: Message) => {
                    if(msg.result) {
                        ...
                        if(sessionStorage.getItem("netology_state_keeper")) {
                            ...
                            // подключаемся к попап и кидаем ему сообщение с данными. Ответа не ожидаем.
